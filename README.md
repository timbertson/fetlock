...
In a corporate setting, you will often need to fetch from private repositories. Nix's builtin suport for `.netrc` authentication is patchy (it only works at _evaluation_ time, it can't be used at build time), so I built [netproxrc](https://github.com/timbertson/netproxrc), which can transparently inject authentication from a `.netrc` into any program which understands `$https_proxy` etc, including fetlock.
...
To inject credentials, run the relevant program under `netproxrc`. e.g. `netproxrc -- fetlock` or `netproxrc -- nix-build`.
