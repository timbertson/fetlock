...
impl PackageJson {
	...
	fn parse<'a>(script: &'a Script<String>) -> Result<Value<'a>> {
		let script: Script<Value<'a>> = script.map_result(|arg| Self::parse_str(&arg))?;
	...
	pub fn build<'c, C: NixCtx>(self, ctx: &C) -> Result<NixBuild> {
		...
		}) = self.esy
		{
			...
			let exported_env_list = exported_env
				.into_iter()
			...
			if !exported_env_list.is_empty() {
				nix_build
					.extra
					.insert("exportedEnv".to_owned(), Expr::List(exported_env_list));
			}

			let build_env_list = build_env
				.into_iter()
			...
			if !build_env_list.is_empty() {
				nix_build
					.extra
					.insert("buildEnv".to_owned(), Expr::List(build_env_list));
			}
		}
		Ok(nix_build)
	}
