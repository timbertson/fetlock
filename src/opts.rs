...
impl CliOpts {
	pub fn from_argv() -> Result<CliOpts> {
		...
		opts.optopt("", "repo-freshness", "maximum age (in days) for cached repos", "DAYS");
		...
		let lock_type = match matches.opt_str("type").unwrap_or_else(||"AUTODETECT_NOT_IMPLEMENTED".to_owned()).as_str() {
			"esy" => lock::Type::Esy,
			"yarn" => lock::Type::Yarn,
			other => { return Err(anyhow!("Unknown type: {}", other)) },
		};
		let repo_freshness_days = matches.opt_str("repo-freshness")
			.map(|s| str::parse(&s)).transpose()?
			.unwrap_or(1);
