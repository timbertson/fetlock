...
impl Ctx {
	...
	fn resolve<'a, 'c: 'a, Ctx: NixCtx>(
		...
		let resolved = match scope {
			VarScope::Unknown => match ident {
				...
				"os-family" | "os-distribution" => match ctx.pkg_type() {
				...
				ident => {
					...
					.or_else(|_| Self::resolve(ctx, VarScope::SelfScope, ident))
					.or_else(|_| {
			...
			VarScope::SelfScope => match ident {
				...
				ident => Self::resolve_bool(true, ident).map(Ok).unwrap_or_else(|| {
					Self::resolve_path(
						PkgImpl {
							path: Expr::str("$out".to_owned()),
			...
			VarScope::Package(pkg) => {
				Self::resolve_bool(pkg.is_some(), ident)
					.map(Ok)
					.unwrap_or_else(|| {
						// Unimplemented:
	...
	pub fn resolve_varident<'a, 'c: 'a, Ctx: NixCtx>(
		ctx: &'c Ctx,
		ident: &Varident<'a>,
	) -> Result<Eval> {
		...
		match v {
			...
			UnaryOp(op, v) => {
				...
				match op {
					...
					And | Or | Eq | Neq | Lte | Gte | Lt | Gt | Question | Slash | Colon => {
...
mod tests {
	...
	fn test_paths() {
		...
		assert_eq!(
			...
			vec!(
				site_lib(),
				StringComponent::Literal("/stublibs/mypkg".to_owned())
			)
		);
		assert_eq!(
			resolve("toplevel"),
			vec!(
				site_lib(),
				StringComponent::Literal("/toplevel/mypkg".to_owned())
			)
		);
	}
}
