...
fn dependency_name(s: Src) -> Res<Name> {
  map(tuple((
    ...
    char(' '),
    non_newline,
  )), |(_indent, name, _space, _suffix)| Name(name))(s)
}
...
fn section(s: Src) -> Res<Option<Section>> {
  preceded(
    ...
    alt((
      map(
        preceded(tag("GIT"), many0(git_field)),
        |fields| Some(Section::Git(fields.into_iter().filter_map(|x| x).collect()))
      ),
      map(
        preceded(tag("GEM"), many0(gem_field)),
        |fields| Some(Section::Gem(fields))
      ),
      map(
        // discard these
        preceded(alt((
  ...
  specs:
    peiji-san (1.2.0)"#),
      Some(Section::Git(vec!(
        GitField::Common(GemField::Remote("https://github.com/alloy/peiji-san.git".to_owned())),
        GitField::Revision("eca485d8dc95f12aaec1a434b49d295c7e91844b".to_owned()),
        GitField::Common(GemField::Specs(vec!(
          GemSpec {
